...
static void uid_perf_work_remove(struct task_struct *task)
{
	...
	work = kmalloc(sizeof(struct uid_work), GFP_ATOMIC);
	if (!work) {
		/* A dropped remove item must not leave the event installed. */
		spin_lock_irqsave(&uid_perf_lock, flag);
		uid_perf_remove_all_pending = true;
		spin_unlock_irqrestore(&uid_perf_lock, flag);
...
static void uid_perf_remove_all_events(void)
{
	 ...
	 * event release can sleep outside RCU.
	 */
	for (;;) {
		rcu_read_lock();
		next = next_task(task);
...
static int __uid_perf_remove_work(void *unused)
{
	...
	while (!kthread_should_stop()) {
		set_current_state(TASK_RUNNING);
		spin_lock_irqsave(&uid_perf_lock, flag);
		if (uid_perf_remove_all_pending) {
			uid_perf_remove_all_pending = false;
			spin_unlock_irqrestore(&uid_perf_lock, flag);
			uid_perf_remove_all_events();
			spin_lock_irqsave(&uid_perf_lock, flag);
		}
