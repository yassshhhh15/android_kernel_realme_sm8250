 ...
 *                        (protected by binder_deferred_lock)
 * @outstanding_txns:     transactions that must drain before freezing
 *                        (protected by @inner_lock)
 * @is_dead:              process is dead and awaiting free
 *                        when outstanding transactions are cleaned up
 *                        (protected by @inner_lock)
 * @is_frozen:            process rejects new synchronous transactions
 *                        (protected by @inner_lock)
 * @sync_recv:            sync transaction state since the last freeze
 *                        (protected by @inner_lock)
 * @async_recv:           async transaction state since the last freeze
 *                        (protected by @inner_lock)
 * @freeze_wait:          waiters for outstanding transactions to drain
 ...
 * @oneway_spam_detection_enabled: process requested oneway spam reports
 *
...
static void binder_dec_outstanding_txn_ilocked(struct binder_proc *proc)
{
...
static void binder_free_transaction(struct binder_transaction *t)
{
	...
	if (target_proc) {
		...
		binder_dec_outstanding_txn_ilocked(target_proc);
		if (t->buffer)
 ...
 *		BR_FROZEN_REPLY if a synchronous transaction was rejected
 *		BR_TRANSACTION_PENDING_FROZEN if an asynchronous transaction
 *		was queued to a frozen target process
 */
				    ...
				    struct binder_thread *thread)
{
	...
	if ((frozen && !oneway) || proc->is_dead ||
		...
		binder_inner_proc_unlock(proc);
		binder_node_unlock(node);
		return frozen && !oneway ? BR_FROZEN_REPLY : BR_DEAD_REPLY;
	}
#ifdef CONFIG_OPLUS_BINDER_STRATEGY
	...
	/*
	 * Free the outdated transaction and buffer after releasing the locks
				 ...
				 struct binder_handle_cookie *handle_cookie)
{
	...
	if (freeze->cookie != handle_cookie->cookie) {
		binder_user_error("%d:%d BC_CLEAR_FREEZE_NOTIFICATION cookie %016llx/%016llx\n",
				 ...
				 struct binder_thread *thread)
{
	...
	while (t) {
		...
		if (t->to_thread == thread) {
			binder_dec_outstanding_txn_ilocked(thread->proc);
			t->to_proc = NULL;
			t->to_thread = NULL;
			if (t->buffer) {
				t->buffer->transaction = NULL;
...
static long binder_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case BINDER_FREEZE: {
		...
		struct binder_proc **target_procs, *target_proc;
		...
		target_procs = kcalloc(target_procs_count,
				       sizeof(*target_procs), GFP_KERNEL);
		if (!target_procs) {
			mutex_unlock(&binder_procs_lock);
			ret = -ENOMEM;
			goto err;
				     ...
				     struct binder_work *w)
{
	...
	case BINDER_WORK_TRANSACTION_COMPLETE:
		...
		break;
	case BINDER_WORK_TRANSACTION_PENDING:
		seq_printf(m, "%stransaction pending\n", prefix);
		break;
	case BINDER_WORK_TRANSACTION_ONEWAY_SPAM_SUSPECT:
		seq_printf(m, "%soneway spam suspect\n", prefix);
		break;
	case BINDER_WORK_NODE:
